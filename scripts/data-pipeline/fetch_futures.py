...
def filter_data_by_date(df, start_date: str, end_date: str) -> pd.DataFrame:
    """
    根据日期范围过滤期货数据
    ...
    # 查找日期列 - 常见列名
    date_columns = ['日期', 'date', '时间', 'time', 'datetime', 'trade_date']
    date_col = None
    ...
    df_copy = df.copy()
    try:
        # 如果日期格式为 YYYY-MM 或 YYYY-MM-DD 或 YYYYMMDD
...
def get_futures_data(symbol: str, exchange: str, start_date: str = None, end_date: str = None) -> dict:
    ...
    """
    # 构建缓存key
    cache_key = f"futures:{symbol}:{exchange}:{start_date}:{end_date}"

    # 尝试从缓存读取
    if _cache_available:
        cached = _cache.get(cache_key, TTL_MARKET)
        if cached is not None:
            print(f"[CACHE] 从缓存读取 {symbol}({exchange}) 数据成功")
            return cached
...
def save_to_csv(data, symbol: str, exchange: str, output_dir: str, start_date: str = None, end_date: str = None) -> str:
    """保存期货数据到CSV"""
