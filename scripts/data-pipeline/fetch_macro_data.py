...
def fetch_gdp_data(region: str = "CN", start: str = None, end: str = None) -> dict:
    ...
    try:
        if region == "CN":
            ...
            return {"status": "success", "data": df, "indicator": "GDP", "region": region}
        elif region == "US":
            # 美国GDP数据
            df = ak.macro_usa_gdp()
...
def fetch_cpi_data(region: str = "CN", start: str = None, end: str = None) -> dict:
    ...
    try:
        if region == "CN":
            ...
            return {"status": "success", "data": df, "indicator": "CPI", "region": region}
        elif region == "US":
            # 美国CPI数据
            df = ak.macro_usa_cpi()
...
def fetch_ppi_data(region: str = "CN", start: str = None, end: str = None) -> dict:
    ...
    try:
        if region == "CN":
            ...
            return {"status": "success", "data": df, "indicator": "PPI", "region": region}
        else:
            return {"status": "error", "message": f"不支持的地区代码: {region}"}
    except Exception as e:
...
def fetch_pmi_data(region: str = "CN") -> dict:
    ...
    try:
        if region == "CN":
            ...
            return {"status": "success", "data": df, "indicator": "PMI", "region": region}
        else:
            return {"status": "error", "message": f"不支持的地区代码: {region}"}
    except Exception as e:
...
def save_data(result: dict, output_dir: str):
    """
    将数据保存到CSV文件
